CREATE TABLE Departments (
	Id			UNIQUEIDENTIFIER NOT NULL PRIMARY KEY,
	Name		NVARCHAR(50) NOT NULL
) ;

INSERT INTO Departments 
	( Id, Name )
VALUES 
	( 'D3C376E4-BCE3-4D85-ABA4-E3CF49612C94',  N'IT одтел'		 	 ), 
	( '131EF84B-F06E-494B-848F-BB4BC0604266',  N'Бухгалтерия'		 ), 
	( '8DCC3969-1D93-47A9-8B79-A30C738DB9B4',  N'Служба безопасности'), 
	( 'D2469412-0E4B-46F7-80EC-8C522364D099',  N'Отдел кадров'		 ),
	( '1EF7268C-43A8-488C-B761-90982B31DF4E',  N'Канцелярия'		 ), 
	( '415B36D9-2D82-4A92-A313-48312F8E18C6',  N'Одтел продаж'		 ), 
	( '624B3BB5-0F2C-42B6-A416-099AAB799546',  N'Юридическая служба' )

	SELECT * FROM Departments AS D -- псевдоним таблиц
	SELECT D.* FROM Departments AS D 
	SELECT D.Id, D.Name FROM Departments AS D 
	
	SELECT D.Id AS dep_id, D.Name AS dep_name FROM Departments AS D 
	-- AS - опционально ->
	SELECT 
		D.Id  dep_id,
		D.Name  dep_name 
	FROM Departments D 
	-- Если псевдонимы составлены не по правилам именования их нужно взять в []
	SELECT
		D.Id  [Код отдела],
		D.Name  [Название]
	FROM Departments D
	
	SELECT TOP 2 -- оставить только 2 первых результата *
		D.Id   [Код отдела],
		D.Name   [Название] 
	FROM Departments D
	